...
const ClinicalServicesSection = () => (
  <section
    id="services"
    aria-labelledby="clinical-services-heading"
    className="bg-cream-dark dark:bg-dark-surface py-24 md:py-32 px-6 transition-colors duration-300"
  >
    <div className="max-w-5xl mx-auto">
      <div className="text-center mb-20">
        ...
        <MaskedReveal delay={0.1}>
          <SectionHeading id="clinical-services-heading" className="mb-4">Therapeutic Services</SectionHeading>
        </MaskedReveal>
        <InfinityDivider delay={0.15} />
        <Reveal delay={0.2}>
          <p className="text-brand-deep/60 dark:text-cream/50 max-w-lg mx-auto">
            Every service is rooted in compassion and the belief that you
            already have what you need to heal.
          </p>
        </Reveal>
      </div>
          ...
          <Reveal key={s.title} delay={i * 0.1}>
            ...
            >
              ...
              <div className="relative z-2">
                <AnimatedIcon
                  delay={i * 0.1 + 0.15}
                  className="text-brand-accent mb-5"
                >
                  <div className="w-10 h-10 rounded-full border border-brand-accent/20 flex items-center justify-center">
                    {s.icon}
                  </div>
                </AnimatedIcon>
                <h3 className="text-xl font-pt-serif text-brand-deep dark:text-cream tracking-wide mb-3">
                  {s.title}
                </h3>
                <p className="text-brand-deep/65 dark:text-cream/55 leading-relaxed mb-4 text-sm">
                  {s.desc}
                </p>
                <motion.p
